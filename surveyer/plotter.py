...
class ReportMaker:
            ...
            if chap.find("chap")>-1:
                ...
                for subkey in self.conf[chap]:
                    ...
                    if "count" in subkey:
                        ...
                        plotname = self.make_count_chart(cluster["identifier"], charttype, title, altanswers)
                        if plotname:
                            if os.path.exists(self.outpath + "pages/figures/"+ plotname):
                    ...
                    if "compare" in subkey:
                        ...
                        if "altoptions" in cluster:
                            altanswers = cluster["altoptions"]
                        plotname = self.make_compare_likert(cluster["identifier"], cluster["selectedparts"], title, altanswers)
                        if plotname:
                            if os.path.exists(self.outpath + "pages/figures/"+ plotname):
                                os.remove(self.outpath + "pages/figures/"+ plotname)
                            shutil.move(plotname, self.outpath + "pages/figures/")
                    ...
                    if "multirate" in subkey:
                        ...
                        plotname = self.make_multirating(cluster["identifier"], title, exclusion)
                        if plotname:
                            if os.path.exists(self.outpath + "pages/figures/"+ plotname):
                                os.remove(self.outpath + "pages/figures/"+ plotname)
                            shutil.move(plotname, self.outpath + "pages/figures/")
    ...
    def make_count_chart(self, questionid, charttype = "bar", title = "", altanswers = []):
        ...
        if self.dataset.metadata["A2"]["entrytype"] == "enumerate":
            ...
            df_enlisted = [entry.split("; ") for entry in df_nona[df_extract.columns[0]]]
            df_extract = pd.DataFrame({df_extract.columns[0]: [item for sublist in df_enlisted for item in sublist]})
        ...
        if altanswers:
            if len(answer_counts) == len(altanswers):
                if charttype != "pie":
                    plt.xticks(ticks=range(len(altanswers)), labels=altanswers, rotation=40, ha='right')
                else:
                    plt.pie(answer_counts, labels=altanswers, autopct='%1.1f%%', startangle=90)
            else:
                print ("Alternative options length", len(altanswers), "does not match required length", len(answer_counts))
    
        filename = "plot_"+charttype + "_" + str(questionid).replace("[", "").replace("]", "").replace(",", "_").replace("'", "")  + ".png"
    ...
    def make_compare_likert(self, questionid, subquestion, title = "", altanswers = []):
        ...
        #df_extract = self.dataset.extract_subset(questionids, acceptedtypes) # also extract subquestion
        ...
        if altanswers:
            ...
            altnames = [str(ent).replace("['","").replace("']","").replace("','"," : ") for ent in altnames]
        ...
        for col in columns:
            if col in self.dataset.metadata[questionid]["params"]["options"]: 
                if not options:
                    ...
                    newopts = self.dataset.metadata[questionid]["params"]["options"][col]
                    for opt in newopts:
            ...
            if col in self.dataset.metadata[questionid]["params"]["subtypes"]:
                if not plottype:
                    plottype = self.dataset.metadata[questionid]["params"]["subtypes"][col]
                else:
                    if self.dataset.metadata[questionid]["params"]["subtypes"][col] != plottype:
                        print ("Question types don't match!",self.dataset.metadata[questionid]["params"]["subtypes"][col], plottype)
                        plottype = "mismatch"
        ...
        sns.countplot(data=df_melted, x=subquestion, hue='Response', order=df.columns, hue_order=scale, palette='coolwarm')
        
        # Add titles and labels
        plt.title(title)
        plt.xlabel('Question')
        plt.ylabel('Count')
        plt.legend(title=subquestion, bbox_to_anchor=(1.05, 1), loc='upper left')  # Move the legend outside
        plt.xticks(ticks=range(len(altnames)), labels=altnames)  # Rotate x-axis labels if neede
    
        plt.tight_layout()  # Adjust layout for readability
    
        filename = "plot_"+subquestion.replace(" ", "_") + "_" + str(questionid) + ".png"
    ...
    def make_rating(self, questionid, title = ""):
        ...
        all_ratings = list(range(1, self.dataset.metadata[questionid]["params"]["options"]["factor"]+1))
    
        df_extract = pd.DataFrame({df_extract_all.columns[0]: [int(entry.split("/")[0]) for entry in df_extract]}).dropna()
        df_extract = pd.to_numeric(df_extract[df_extract_all.columns[0]], errors='coerce')
    ...
    def make_multirating(self, questionid, title = "", excludeterms = []):
        ...
        altnames = [str(entry[0]) for entry in self.dataset.metadata[questionid]["subquestions"]]
        ...
        # Plot each question as a horizontal bar with error bars for standard deviation
        plt.figure(figsize=(8, 6))
        plt.barh(y=means.index, width=means, xerr=stds, color='skyblue', edgecolor='gray')
        ...
        plt.xlabel("Rating out of "+str(self.dataset.metadata[questionid]["params"]["options"]["factor"]))
        plt.title(title)
        plt.grid(axis='x', linestyle='--', alpha=0.7)

        plt.tight_layout()

        filename = "plot_multirating_"+ str(questionid) + ".png"
